...
static int updateWordInTallyList(LLNode **wordListHeads, int maxLen, char *word)
{
	...
	LLNode *wordFound = llLookupKey(wordListHeads[wordLen - 1], word);
	...
	LLNode *newList = llNewNode(strdup(word), 1);
	if (newList != NULL)
	{
		wordListHeads[wordLen - 1] = llAppend(wordListHeads[wordLen - 1], newList);
	}
	else
	{
		return 0;
